"""
Main application file with integrated enhancements.
This version includes the enhanced document types and optimized API client.
"""
...
def initialize_session_state():
    ...
    if not hasattr(st.session_state, "metadata_config"):
        st.session_state.metadata_config = {
            ...
            "batch_size": 10  # Increased from 5 for better performance
        }
    ...
    if not hasattr(st.session_state, "processing_state"):
        st.session_state.processing_state = {
            ...
            "retry_delay": 2,
            "visualization_data": {},
            "parallel_processing": True,  # Enable parallel processing by default
...
with st.sidebar:
    ...
    if hasattr(st.session_state, "authenticated") and st.session_state.authenticated:
        ...
        with st.expander("Performance Settings", expanded=False):
            ...
            # Apply performance settings to processing state
            if "processing_state" in st.session_state:
                st.session_state.processing_state["parallel_processing"] = st.session_state.performance_settings["parallel_processing"]
                st.session_state.processing_state["max_workers"] = st.session_state.performance_settings["max_workers"]
            
            # Apply performance settings to document categorization
            if "document_categorization" in st.session_state and "processing_state" in st.session_state.document_categorization:
                st.session_state.document_categorization["processing_state"]["parallel_processing"] = st.session_state.performance_settings["parallel_processing"]
                st.session_state.document_categorization["processing_state"]["max_workers"] = st.session_state.performance_settings["max_workers"]
                st.session_state.document_categorization["processing_state"]["batch_size"] = st.session_state.performance_settings["batch_size"]
            
            # Apply performance settings to metadata config
            if "metadata_config" in st.session_state:
                st.session_state.metadata_config["batch_size"] = st.session_state.performance_settings["batch_size"]
    ...
    if not hasattr(st.session_state, "current_page") or st.session_state.current_page == "Home":
        ...
        with col1:
            ...
            workflow_steps = [
                ...
                {"icon": "🏷️", "title": "Configure Document Types", "description": "Define custom document types for categorization"},
                {"icon": "🔍", "title": "Categorize Documents", "description": "Use AI to automatically categorize your documents"},
                {"icon": "⚙️", "title": "Configure Metadata", "description": "Set up metadata extraction parameters"},
                {"icon": "🤖", "title": "Process Files", "description": "Extract metadata from selected files using AI"},
                {"icon": "👁️", "title": "Review Results", "description": "View and edit extracted metadata"},
                {"icon": "✅", "title": "Apply Metadata", "description": "Apply extracted metadata to your Box files"}
            ]
            
            for step in workflow_steps:
                st.write(f"{step['icon']} **{step['title']}**: {step['description']}")
        
        with col2:
            st.write("### Features")
            st.write("✨ **Enhanced Document Types**")
            st.write("- Custom document types with examples")
            st.write("- Improved categorization accuracy")
            st.write("- Structured document type handling")
            
            st.write("🚀 **Performance Optimizations**")
            st.write("- Parallel processing")
            st.write("- Enhanced caching")
            st.write("- Optimized API requests")
            st.write("- Adaptive batch processing")
